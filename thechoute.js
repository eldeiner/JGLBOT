...
//CREADOR
const yo = `JGL Deiner`
const Githu = 'https://github.com/eldeiner'
const Youtube = 'https://www.youtube.com'
const WhatsApp = '31075878474'
const instagram = 'https://www.instagram.com/eldeiner.c'
...
const getRegisteredRandomId = () => {
  ...
  try {pushnem = choute.key.fromMe ? leo.user.name : conts.notify || conts.vname || conts.name || '-'
    ...
    ppimg = 'https://i.ibb.co/2n8R25c/JGL.png'}  
    
     teks = 
  `😙Hola, @${num.split('@')[0]}
  Bienvenido a JGL OFICIAL ${mdata.subject}
...
isCharge: "" || false
 ...
 const rechoute = async(id, título, descripción, buffer, enlace) => {
    ...
    leo.sendMessage(id, {name: "JGL-BOT", address: "", jpegThumbnail: fs.readFileSync('./media/imagen/Faketumber.jpg'), }, MessageType.text, opción)}
...
const command = comm
  ...
  const ownerNumber  = "3107587847@s.whatsapp.net"
...
const selocont = { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: '3107857847@s.whatsapp.net' } : {}) }, message: { "contactMessage": { "displayName": `${pushname}`, "vcard": `BEGIN:VCARD\nVERSION:3.0\nN:XL;${pushname},;;;\nFN:${pushname},\nitem1.TEL;waid=${sender.split('@')[0]}:${sender.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`, "jpegThumbnail":fs.readFileSync(`./media/imagen/izu.jpg`)
        }}};
...
*Instagram:*
https://www.instagram.com/eldeiner.c/
*Github:*
https://github.com/eldeiner`
...
its = await getBuffer (ppimg)
    ...
    "sourceUrl": 'https://www.instagram.com/eldeiner.c/'}}})
      addFilter(from)  
  addLevelingLevel(sender, 5)	}}
...
if (choute.message.listResponseMessage){
  ...
  `${texto}\n${allmenu}`
    ...
    buttonsMessage = {
    contentText: `${txtt}`,
    footerText: '© Creator JGL OFICIAL', imageMessage: imageMsg,
    buttons: buttons1,
    headerType: 4}
...
addLevelingLevel(sender, 5)	}}
  ...
  if (choute.message.listResponseMessage){
    test = choute.message.listResponseMessage.singleSelectReply.selectedRowId
    if (test.includes(`entretenimiento`)){
    leo.updatePresence(from, Presence.composing)
    if (!isRegister) return leoply(baby.only.usrReg)
    uptime = process.uptime()
    ...
    if (choute.message.buttonsResponseMessage){
      ...
      if (test.includes(`hetai`)){
        ...
        bufhtz = await getBuffer(hentaiz.data.url)
          ...
          buttonsMessage = {
          contentText: `${txtt}`,
          footerText: 'Deiner \n© Creator JGL OFICIAL', imageMessage: imageMsg,
          buttons: buttons1,
          headerType: 4}
          prep = leo.prepareMessageFromContent(from, { buttonsMessage }, { quoted: choute,  contextInfo: { mentionedJid: [sender,ownerNumber]} })
          leo.relayWAMessage(prep)
...
if (body.includes(`@${botNumber}`)){
  ...
  async function extraPlay(cArgs) {
    ...
    ✅ *Link de canal:* ${res.all[0].author.url}
...
case 'creador':
                ...
                ppimg = 'https://i.ibb.co/862vqR3/369933542-327262276607060-6206582779495462995-n.jpg'}
...
case 'reglas':
                if (isBan) return leoply  (baby.only.benned)	
                if (!isRegister) return leoply(baby.only.usrReg)	
                mreg = leo.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 9999999, "message": `${reglas}`, "footerText": "*JGL OFICIAL*", "thumbnail": fs.readFileSync('./media/imagen/imgregla.jpg'), "surface": 'CATALOG' }}, {contextInfo: { mentionedJid: [sender,ownerNumber]}, quoted:choute, })
